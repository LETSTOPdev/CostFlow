...
`fixtures/` holds the input CSV, mapping template, and assumption set.
`expected/` holds the frozen outputs (`run.json`, `report.md`) that CI compares
byte-exactly against every pipeline run (NFR-1).
