...
Import batch `batch-golden-demo-clickup` (provider: clickup, mapping `clickup-delivery-list` v1, imported 2026-07-20T00:00:00Z)
...
- Lifecycle events imported: 9
- Unmapped actors pseudonymized (scope `costflow-golden`); raw identities are not retained

### Import diagnostics

- File \[warning]: 1 additional assignee(s) beyond the primary were not imported (CU3: one actor per item).

## Detectors

- Aging / stagnation (`f2-aging@1.1.0`): ran — 1 finding(s)
- Queue wait (`f1-queue-wait@1.1.0`): ran — 2 finding(s)
- Overdue exposure (`f3-overdue@1.1.0`): ran — 2 finding(s)

## Ranked frictions

| # | Friction | Where | Magnitude | Estimated cost | Confidence |
|---|---|---|---|---|---|
| 1 | queue-wait | stage "backlog" (queue) | 1584 item-hours-waiting | 555 USD – 2,220 USD (expected ~1,110 USD) | C |
| 2 | overdue | stage "backlog" (queue) | 19 item-days-overdue | 171 USD – 684 USD (expected ~342 USD) | A |
| 3 | aging | stage "backlog" (queue) | 11 item-days-beyond-threshold | 148 USD – 594 USD (expected ~297 USD) | B |
...
- Work-in-flight load (`c6-wip-load@1.0.0`): 2 of 3 in-flight items (67%) sit in queue- or review-kind stages; the largest single pool is stage "backlog" (1 items).

## Drill-down #1: queue-wait at stage "backlog"

**What is this?** Estimated follow-up cost of 4 item(s) waiting in stage "backlog", observed from event history.
...
(cost model `cm-queue-wait-attention@1.0.0`, assumption set `demo-clickup-assumptions` v1)

**What data went in?**

| Item | Wait (days) | Visits | Open at analysis time | Attention h/day | Rate | Subtotal |
|---|---|---|---|---|---|---|
| 86czqc3 "Quarterly access audit" | 49 | 1 | yes | 0.1–0.4 | 90/h (rates.Ops) | 441 USD – 1,764 USD (expected ~882 USD) |
| 86czqb2 "Vendor risk review" | 10 | 1 | no | 0.1–0.4 | 30/h (defaultRate:unmapped-actor) | 30 USD – 120 USD (expected ~60 USD) |
| 86czqa1 "Renew SOC 2 evidence collection" | 5 | 1 | no | 0.1–0.4 | 120/h (rates.Legal) | 60 USD – 240 USD (expected ~120 USD) |
| 86czqd4 "Publish incident postmortem" | 2 | 1 | no | 0.1–0.4 | 120/h (rates.Legal) | 24 USD – 96 USD (expected ~48 USD) |
...
|---|---|---|---|---|---|
| 86czqc3 "Quarterly access audit" | 19 | 2026-07-01T00:00:00.000Z | 0.1–0.4 | 90/h (rates.Ops) | 171 USD – 684 USD (expected ~342 USD) |
...
|---|---|---|---|---|
| 86czqc3 "Quarterly access audit" | 11 | 0.15–0.6 | 90/h (rates.Ops) | 148 USD – 594 USD (expected ~297 USD) |
...
| Item | Wait (days) | Visits | Open at analysis time | Attention h/day | Rate | Subtotal |
|---|---|---|---|---|---|---|
| 86czqa1 "Renew SOC 2 evidence collection" | 12 | 1 | yes | 0.1–0.4 | 120/h (rates.Legal) | 144 USD – 576 USD (expected ~288 USD) |
...
| Item | Days overdue | Due date | Attention h/day | Rate | Subtotal |
|---|---|---|---|---|---|
| 86czqa1 "Renew SOC 2 evidence collection" | 10 | 2026-07-10T00:00:00.000Z | 0.1–0.4 | 120/h (rates.Legal) | 120 USD – 480 USD (expected ~240 USD) |
